...
var { owner, repo } = context2.repo;
var event_type = context2.eventName;
var labels = ["lfs-detected!"];
function run() {
  return __async(this, null, function* () {
    var _a, _b;
    try {
      ...
      try {
        ...
        if (error2.message === "Not Found") {
          ...
          core.error(`getLabel error: ${error2.message}`);
        }
      }
      if (event_type === "pull_request") {
        const pullRequestNumber = (_a = context2.payload.pull_request) == null ? void 0 : _a.number;
        ...
        const prFilesWithBlobSize = yield Promise.all(files.map((file) => __async(this, null, function* () {
          const { filename, sha, patch } = file;
          const { data: blob } = yield octokit.rest.git.getBlob({
            owner,
            repo,
            file_sha: sha
          });
        ...
        for (const file of prFilesWithBlobSize) {
          ...
          if (fileblobsize !== null && fileblobsize > Number(fsl)) {
            ...
            const shouldBeStoredInLFS = (yield execP(`git check-attr filter ${filename}`)).stdout.includes("filter: lfs");
            if (shouldBeStoredInLFS) {
              const isStoredInLFS = Boolean((_b = file.patch) == null ? void 0 : _b.includes("version https://git-lfs.github.com/spec/v1"));
              if (!isStoredInLFS) {
